...
st.set_page_config(page_title="Retail Product Inventory", layout="wide")
st.title("🛍️ Retail Product Inventory Analysis")
...
# --- Keep only wanted subcategories ---
...
df = df[df["subcategory"].isin(retail_subcategories)]
...
st.subheader("📋 Retail Product Inventory Summary")
...
if not top_qty_variance.empty:
    fig1 = px.bar(
        ...
        title="Top 10 Retail Variance by Quantity",
        color="qty_variance",
...
if not top_value_variance.empty:
    fig2 = px.bar(
        ...
        title="Top 10 Retail Variance by $",
        color="variance",
